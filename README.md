...
- **中英双语界面**：页眉右上角「中 / EN」一键切换，界面与地名全部跟随（默认跟随浏览器语言，
  也可用 `?lang=en` 直接进英文）
...
| 路径 | 说明 |
| --- | --- |
| `index.html` | 地图册本体（2.6MB，纯静态，无外部依赖） |
...
| `data/atlas_api.json` | 结构化数据接口：全部城镇与大洲数据（含三种坐标、行省、派系、旗帜格位，**城镇/行省/大洲/派系均含中英双名**）。**其中 `px` 按本站 4000px 底图计算**，与世界坐标/六边形坐标不同，换底图尺寸需重新生成 |
